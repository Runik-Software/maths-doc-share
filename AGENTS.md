...
- Install: `pnpm install` (uses `pnpm` by default)
- Dev: `pnpm dev`
- Build: `pnpm build`
- Start (prod): `pnpm start`
- Run Payload CLI: `pnpm payload`
- Generate types/import map: `pnpm generate:types`, `pnpm generate:importmap`
- Tests: `pnpm test` (runs `test:int` + `test:e2e`)
...
- For schema/collection changes: update `src/collections/*`, then run `pnpm generate:types` and run tests. Avoid destructive DB migrations without coordination.
...
- Testing: run `pnpm run test:int` for quick checks; run e2e (`pnpm run test:e2e`) only when necessary or in CI.
- Formatting/linting: run `pnpm run lint` and `prettier` before commits.
- Avoid committing secrets or `.env` files. Use CI secrets or `.env` templates.

Common tasks and pointers

- Add a content model: update `src/collections/`, add frontend rendering in `src/blocks/` or `src/app/(frontend)/`, run `pnpm generate:types`.
